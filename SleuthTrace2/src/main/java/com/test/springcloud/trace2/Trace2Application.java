package com.test.springcloud.trace2;
 ...
 * 1.Sleuth跟踪支持通过RabbitMQ/Kafka(以及其它Stream绑定器实现的消息中间件)传递的请求、Zuul代理传递的请求和RestTemplate发起的请求,
 * 每个请求发送之前,Sleuth会在请求Header中增加X-B3-TraceId、X-B3-SpanId、X-B3-ParentSpanId、X-B3-Sampled是否抽样输出标志等实现跟踪重要信息<br>
 * 2.输出INFO [trace-2,701cf23a48a2bd2a,08eabe4b0e2b7a15,false]日志信息,第一个值记录了应用名称,
